...
- [ ] T001 [P] Add `formFieldControlCap: numFromEnv("HYPPO_FORM_FIELD_CONTROL_CAP", 200)` and `formFieldOptionCap: numFromEnv("HYPPO_FORM_FIELD_OPTION_CAP", 200)` to the `config` object in `src/main/config.ts`, beside the existing limits.
- [ ] T002 [P] Extend `tests/fixtures/form.html`: add a second `<form id="otherform">` containing exactly one text `<input id="other_field">`; add one `<input type="text">` with **neither `id` nor `name`** inside `#theform` (selector synthesis); add a `<fieldset id="shift">` with two `<input type="radio" name="shift">` radios (`group`). Leave every existing element, id, and `<script>` hook untouched.
