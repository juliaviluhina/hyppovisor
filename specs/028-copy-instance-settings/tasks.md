...
- [ ] T001 Survey `src/renderer/panel.ts` instance list rendering + `src/renderer/snippets.ts` builders and record per-row reuse points in `specs/028-copy-instance-settings/research.md` (append, don't rewrite)
- [ ] T002 [P] Survey `src/main/instances/registry.ts` discovery and `src/main/settings.ts` token/port resolution; note sibling-settings read points in the same research appendix
...
- [ ] T003 Add `RowSettings` / `SettingsBlocks` types in `src/shared/types.ts`
- [ ] T004 Implement sibling settings read (runtime + settings files, unreadable ⇒ unavailable) in `src/main/instances/registry.ts` (or adjacent module per plan)
- [ ] T005 Implement per-row `SnippetState` assembly (server-name derivation, auth-off omission, stdio variant) reusing `src/renderer/snippets.ts` builders
- [ ] T006 [P] Unit tests for assembly: HTTP+auth, auth-off, stdio, server names, corrupt/unreadable files in `tests/unit/instance-settings-copy.test.ts`
...
- [ ] T007 [P] [US1] Unit tests for block contents (endpoint URL, header presence, server-name match, JSON parses) in `tests/unit/instance-settings-copy.test.ts`
- [ ] T008 [P] [US1] E2E: copy sibling row blocks and prove the JSON reaches the sibling instance in `tests/integration/instance-settings-copy.spec.ts`

### Implementation for User Story 1

- [ ] T009 [P] [US1] Add per-row copy buttons (command + JSON) to the instance list in `src/renderer/panel.ts`
- [ ] T010 [US1] Wire row buttons to clipboard via the assembled blocks in `src/renderer/panel.ts` (depends on T005, T009)
...
- [ ] T011 [P] [US2] Unit test: own-row assembly equals panel assembly for identical input in `tests/unit/instance-settings-copy.test.ts`
- [ ] T012 [P] [US2] E2E: three instances, each row's blocks reach the right instance in `tests/integration/instance-settings-copy.spec.ts`

### Implementation for User Story 2

- [ ] T013 [US2] Generalize row assembly beyond background rows (self + foreground siblings) in `src/main/instances/registry.ts` (or adjacent module)
...
- [ ] T014 [P] [US3] Unit tests for unreachable marking and unavailable rows in `tests/unit/instance-settings-copy.test.ts`
- [ ] T015 [P] [US3] E2E: killed sibling shows marked last-known blocks that fail fast in `tests/integration/instance-settings-copy.spec.ts`

### Implementation for User Story 3

- [ ] T016 [US3] Render unreachable/unavailable row states with marked blocks (or no copy) in `src/renderer/panel.ts`
...
- [ ] T017 [P] Document per-row copy in the connection-panel docs (`docs/` connection guide) and the instance-management contract amendment
- [ ] T018 [P] Extend `tests/unit/connection-snippets.test.ts` if new builder variants were added
- [ ] T019 Run `npm test`, `npm run lint`, `npx playwright test instance-settings-copy`, and quickstart.md Scenarios 1–4; fix fallout
- [ ] T020 Checksum side-effect proof (quickstart Scenario 4) as an automated e2e assertion if feasible, else documented manual run
