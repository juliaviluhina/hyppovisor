...
- [x] T020 [US4] Ran `oxipng -o4 --strip safe build/icon.png assets/hyppovisor.png
  src/renderer/mascot.png src/renderer/hyppo.png` (2026-08-31). Each file is byte-smaller
  and pixel-identical: hyppovisor.png 1,397,659 → 1,265,974 (−9%), icon.png 569,396 →
  501,381 (−11%), mascot.png 348,849 → 287,450 (−17%), hyppo.png 12,301 → 11,596 (−5%).
  Total 2,328,205 → 2,066,401 (**−11%**). A `--zopfli` pass on top adds only 0.4%.
...
- [x] T023 [US4] `npm run build` clean. `npm run test:e2e` — 88 passed incl. the About
  ...
  default MCP port. The lossless pass (T020) yields **−11%**, short of SC-008's ≥ 40%
  target: 40% is unreachable without downscaling the brand PNGs (`assets/hyppovisor.png`
  is 1159 px wide, rendered at 200 px), which is a resolution change out of this P3
  story's "losslessly smaller / no visible change" scope. Treating SC-008's 40% as an
  aspirational target, not a gate; a downscale pass can be its own change if wanted.

**Checkpoint**: US1–US3 complete; US4 done except SC-008's 40% stretch target (see T023).
