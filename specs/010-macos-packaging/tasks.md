...
- [x] T020 [US4] Shrunk the four tracked PNGs (2026-08-31). Lossless `oxipng -o4 --strip
  safe` alone was −11% (2,328,205 → 2,066,401), short of SC-008. Then downscaled the two
  oversized full-figure renders to 3× their on-screen size — `assets/hyppovisor.png`
  1159×1358 → 600×703 (README renders it at 200 px), `src/renderer/mascot.png` 478×560 →
  240×281 (panel renders it at 72 px) — and re-ran `oxipng`. `build/icon.png` (1024²
  icon master) and `src/renderer/hyppo.png` (96²) are lossless-only. Result: hyppovisor.png
  1,397,659 → 422,784, icon.png 569,396 → 501,381, mascot.png 348,849 → 94,181, hyppo.png
  12,301 → 11,596. Total **2,328,205 → 1,029,942 (−55%)**. No visible change in the app or
  README (both downscales keep a 3× margin). `assets/BRANDING.md` updated to match.
...
- [x] T023 [US4] `npm run build` clean; re-run after the downscale copied the new
  `mascot.png` / `hyppo.png` into `dist/`. `npm run test:e2e` — 88 passed incl. the About
  mascot render (`connection-panel.spec.ts` US4, `#panel-mascot`); 4 failures in
  `connection-panel.spec.ts` (US2/US3/US5/§7) are a port collision with a running
  HyppoVisor instance holding `127.0.0.1:7357`, not a regression — those tests bind the
  default MCP port. Tracked-PNG bytes: 2,328,205 → 1,029,942 = **−55%**, clears SC-008's
  ≥ 40% target. README figure, About mascot, and top-bar icon all still render (no visible
  change — every render keeps a ≥ 3× resolution margin).
